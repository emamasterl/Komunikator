...
#test = data2['routes'][0]['legs'][0]['steps'][1]['transit_details']['departure_time']['text']
#action()
#pp.pprint(action())
#pp.pprint(test)
'''
class Pot:
    ...
    def cas(self, x):
        ...
        res = requests.get(url + 'origins=' + origin + '&destinations=' + self.kraj + '&mode=' + nacin3 + '&key=' + key)
        data = res.json()
        sek = data['rows'][0]['elements'][0]['duration']['value'] 
        print(sek)      
        return sek


    def vrni(self):
        nacin2 = self.cas('walking')
        nacin1 = self.cas('transit')
        b = nacin1/60
        a = nacin2/60
        return a, b
